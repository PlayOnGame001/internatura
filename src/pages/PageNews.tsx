	...
	return (
		<div className="min-h-screen p-6">
			...
			<div className="flex flex-wrap gap-5 justify-center">
				{newsData.map((news) => (
					<NewsCard key={news.id} title={news.title} icon={NewsIcon} onClick={() => setSelectedNews(news)}/>
				))}
			</div>
			...
			{selectedNews && (
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
					<div className="bg-gray-400 rounded-lg shadow-lg border-4 border-black p-6 relative max-w-lg w-full mx-4">
						<button type="button" className="absolute top-3 right-3 text-black hover:text-gray-800 text-xl font-bold bg-gray-200 rounded-full w-8 h-8 flex items-center justify-center"
